...
jobjectArray Java_uz_test_debugnative_tmp_NativeUtils_getIfconfig(JNIEnv* env)
{

	int sd;
    if ((sd = socket (AF_INET, SOCK_DGRAM, 0)) < 0) {
        __android_log_print(ANDROID_LOG_DEBUG, "openssl", "Opening socket for intface get failed");
    ...
    if (ioctl (sd, SIOCGIFCONF, &ifc) < 0) {
        __android_log_print(ANDROID_LOG_DEBUG, "openssl", "IOCTL for intface get failed");
    ...
    */
    ...
    for (struct ifreq* ifr = ifc.ifc_req; ifr <   ifs + num_intf; ifr++) {
        ...
        if (ifr->ifr_addr.sa_family != AF_INET)  {
            __android_log_print(ANDROID_LOG_DEBUG, "openssl", "NOT AF_INET: %s", ifr->ifr_name);
            continue;
        }
        ...
        if ((err=getnameinfo(&ifr->ifr_addr, sizeof(struct sockaddr_in), buf, NI_MAXHOST, NULL, 0,
                             NI_NUMERICHOST)) !=0) {
            __android_log_print(ANDROID_LOG_DEBUG, "openssl", "getnameinfo failed for  %s: %s", ifr->ifr_name,  gai_strerror(err));
            continue;
        }
        ...
        if (ioctl (sd, SIOCGIFFLAGS, &ifreq) < 0) {
            __android_log_print(ANDROID_LOG_DEBUG, "openssl", "SIOCGIFFLAGS failed for %s: %s", ifr->ifr_name, strerror(errno));
            continue;
        }
        
        if (!(ifreq.ifr_flags & IFF_UP)) {
            __android_log_print(ANDROID_LOG_DEBUG, "openssl", "IFF_UP failed for %s", ifr->ifr_name);
            continue;
        }

        /* interface netmask */
        if (ioctl (sd, SIOCGIFNETMASK, &ifreq) < 0) {
            __android_log_print(ANDROID_LOG_DEBUG, "openssl", "SIOCIFNETMASK failed for %s: %s", ifr->ifr_name, strerror(errno));
            continue;
        }
